# Minimal Hyprland Noctalia Rice

![My Desktop](./screenshots/observer_8_4_2026)

Slight visual changes after installing the default Hyprland Noctalia configs from CachyOS. Otherwise a few config tweaks for monitor setups so I can usethe same dotfiles on a laptop and a 4 monitor desktop.

Window Manager: Hyprland + Noctalia
Shell: Fish
Terminal: Kitty
Editor: Neovim (Minimal setup)
Wallpapers: Stored in /Pictures/Wallpapers. (Wallpapers were sourced form a few github repos created by others)

!
